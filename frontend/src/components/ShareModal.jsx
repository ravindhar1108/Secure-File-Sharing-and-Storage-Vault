...
import api from '../api';
import { X, Link as LinkIcon, ShieldAlert, Globe, Lock } from 'lucide-react';
...
export default function ShareModal({ file, onClose }) {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      const payload = {
         shareType: shareType,
         password: shareType === 'PRIVATE' ? (password || null) : null,
         expiryHours: shareType === 'PRIVATE' ? (parseInt(expiryHours) || 24) : null,
         maxDownloads: shareType === 'PRIVATE' ? (maxDownloads ? parseInt(maxDownloads) : null) : null,
         viewOnce: shareType === 'PRIVATE' ? viewOnce : false
      };
          ...
          <div>
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', marginBottom: '24px' }}>
              <button 
                ...
                style={{ flex: 1, padding: '12px', background: 'none', border: 'none', borderBottom: shareType === 'PUBLIC' ? '2px solid var(--primary-color)' : '2px solid transparent', color: shareType === 'PUBLIC' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: shareType === 'PUBLIC' ? '500' : '400', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer', transition: 'all 0.2s' }}
              >
                <Globe size={16} /> Public Link
              </button>
              <button 
                onClick={() => setShareType('PRIVATE')} 
                style={{ flex: 1, padding: '12px', background: 'none', border: 'none', borderBottom: shareType === 'PRIVATE' ? '2px solid var(--primary-color)' : '2px solid transparent', color: shareType === 'PRIVATE' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: shareType === 'PRIVATE' ? '500' : '400', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer', transition: 'all 0.2s' }}
              >
                <Lock size={16} /> Secure Link
              </button>
            </div>

            <form onSubmit={handleGenerate} className="flex" style={{ flexDirection: 'column', gap: '16px' }}>
              {shareType === 'PUBLIC' ? (
                <div style={{ padding: '16px', background: 'var(--bg-color)', borderRadius: 'var(--radius)', color: 'var(--text-secondary)', fontSize: '14px', lineHeight: '1.5', textAlign: 'center', marginBottom: '8px' }}>
                   Anyone with this link will be able to view and download the file indefinitely. No password or restrictions will be applied.
                </div>
              ) : (
                <>
                  <div>
                    ...
                    <input type="password" placeholder="Leave blank for open access" className="input-field" value={password} onChange={(e) => setPassword(e.target.value)} />
                  </div>
              ...
              <button type="submit" className="btn btn-primary" style={{ marginTop: '16px' }} disabled={loading}>
                <LinkIcon size={16} /> {loading ? 'Generating...' : `Generate ${shareType === 'PUBLIC' ? 'Public' : 'Secure'} Link`}
              </button>
            </form>
          </div>
        ) : (
